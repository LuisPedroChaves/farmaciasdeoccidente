...
export class NewPayrollComponent implements OnInit {
  ...
  getTotal(): number {
    return this.payrollEmployees.reduce((partial_sum, p) =>  {
      const etotal = (((p._employeeJob as any).initialSalary) + (p.extraHours + p.holiday + p.incentiveBonus + p.jobBonus + p.otherBonus) - (p.igss) - (p.productCharges + p.credits + p.foults))
