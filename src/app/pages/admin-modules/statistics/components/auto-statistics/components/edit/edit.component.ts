import { Component, EventEmitter, Input, OnInit, Output, ViewChild, AfterContentInit, OnDestroy, SimpleChanges, OnChanges } from '@angular/core';
...
import { AutoStatisticService } from 'src/app/core/services/httpServices/auto-statistic.service';
...
export class EditComponent implements OnInit, AfterContentInit, OnDestroy, OnChanges {

  @Input() autoStatistic: AutoStatisticItem;
  ...
  constructor(
    ...
    this.brandsSubscription = this.brandService.readData().subscribe((data) => {
      ...
      this.autoStatistic.brands.forEach(b => {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.autoStatistic) {
      const AUTO_STATISTIC: AutoStatisticItem = changes.autoStatistic.currentValue;
      this.form1.controls.name.setValue(AUTO_STATISTIC.name)
      this.form1.controls.hour.setValue(AUTO_STATISTIC.hour)
      this.form1.controls.minute.setValue(AUTO_STATISTIC.minute)
      this.form1.controls.cellars.setValue(AUTO_STATISTIC.cellars)
      this.form3.controls.daysRequest.setValue(AUTO_STATISTIC.daysRequest)
      this.form3.controls.daysSupply.setValue(AUTO_STATISTIC.daysSupply)
      this.form3.controls.note.setValue(AUTO_STATISTIC.note)
      this.form3.controls.activated.setValue(AUTO_STATISTIC.activated)
      this.selectedBrands = [];
      this.selectedBrandsPage = [];
      this.brandService.loadData();
      AUTO_STATISTIC.brands.forEach(b => {
        this.selectBrand(b);
      })
    }
  }
  ...
  saveAutoStatistic() {
    this.loading = true;
    this.autoStatisticService.update({
      _id: this.autoStatistic._id,
      ...this.form1.value,
